USE [sqldb-ods-dev]
GO
/****** Object:  StoredProcedure [StgDM].[SrcDIMProductCategory]    Script Date: 28.10.2022 12:27:46 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [StgDM].[SrcDIMProductCategory] @CutOffDateFrom VARCHAR(100)
AS
...
FROM (
	...
	FROM (
		SELECT
			...
			sty.SourceSystemCode AS SourceSystemSKcode
			...
			,NULL AS ProductCategoryFullyQualifiedName --not available in ODS ?should be null?
		...
		SELECT
			...
			,NULL AS ProductCategoryFullyQualifiedName --not available in ODS 
		...
		SELECT
			...
			--,RowLastUpdatedBySK	  Populated in ADF
			--,RowLastUpdatedDtm	  Populated in ADF
			sty.SourceSystemCode AS SourceSystemSK
