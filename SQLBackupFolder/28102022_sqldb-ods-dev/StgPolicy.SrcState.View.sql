USE [sqldb-ods-dev]
GO
/****** Object:  View [StgPolicy].[SrcState]    Script Date: 28.10.2022 11:09:26 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	FROM (
		...
		SELECT hdr.SourceSystemCode
			,pol.GoverningStateCode AS StateCode
			,ISNULL(pol.GoverningStateDesc, '') AS StateName
		FROM StgPolicy.PolicyData line
		CROSS APPLY OPENJSON(line.JsonLine) WITH (
				Policy NVARCHAR(MAX) AS JSON
				,SourceSystemCode NVARCHAR(MAX)
		...
		CROSS APPLY OPENJSON(hdr.Policy) WITH (Insured NVARCHAR(MAX) AS JSON
		,GoverningStateCode NVARCHAR(MAX)
		,GoverningStateDesc NVARCHAR(MAX)) pol
		WHERE ISNULL(pol.GoverningStateCode, '') <> ''
