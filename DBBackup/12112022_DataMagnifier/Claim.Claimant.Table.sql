USE [DataMagnifier]
GO
/****** Object:  Table [Claim].[Claimant]    Script Date: 12.11.2022 09:01:17 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Claim].[Claimant]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_ClaimCurrencySK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
ALTER TABLE [Claim].[Claimant]  WITH CHECK ADD  CONSTRAINT [FK_ClaimCurrencySK_Claimant] FOREIGN KEY([ClaimCurrencySK])
REFERENCES [TypeList].[Currency] ([CurrencySK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_ClaimCurrencySK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
ALTER TABLE [Claim].[Claimant] CHECK CONSTRAINT [FK_ClaimCurrencySK_Claimant]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_ClaimSK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_ClaimSK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
ALTER TABLE [Claim].[Claimant] CHECK CONSTRAINT [FK_ClaimSK_Claimant]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_PartySK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
ALTER TABLE [Claim].[Claimant]  WITH CHECK ADD  CONSTRAINT [FK_PartySK_Claimant] FOREIGN KEY([PartySK])
REFERENCES [Party].[PartyMaster] ([PartySK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_PartySK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
ALTER TABLE [Claim].[Claimant] CHECK CONSTRAINT [FK_PartySK_Claimant]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_SourceSystemSK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
ALTER TABLE [Claim].[Claimant]  WITH CHECK ADD  CONSTRAINT [FK_SourceSystemSK_Claimant] FOREIGN KEY([SourceSystemSK])
REFERENCES [TypeList].[SourceSystem] ([SourceSystemSK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_SourceSystemSK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
ALTER TABLE [Claim].[Claimant] CHECK CONSTRAINT [FK_SourceSystemSK_Claimant]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_VehicleStateSK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
ALTER TABLE [Claim].[Claimant]  WITH CHECK ADD  CONSTRAINT [FK_VehicleStateSK_Claimant] FOREIGN KEY([VehicleStateSK])
REFERENCES [TypeList].[State] ([StateSK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_VehicleStateSK_Claimant]') AND parent_object_id = OBJECT_ID(N'[Claim].[Claimant]'))
