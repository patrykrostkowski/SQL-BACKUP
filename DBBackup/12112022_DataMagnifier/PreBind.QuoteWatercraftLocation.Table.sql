USE [DataMagnifier]
GO
/****** Object:  Table [PreBind].[QuoteWatercraftLocation]    Script Date: 12.11.2022 09:01:19 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PreBind].[QuoteWatercraftLocation]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteWatercraftSK_QuoteWatercraftLocation]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteWatercraftLocation]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteWatercraftSK_QuoteWatercraftLocation]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteWatercraftLocation]'))
ALTER TABLE [PreBind].[QuoteWatercraftLocation] CHECK CONSTRAINT [FK_QuoteWatercraftSK_QuoteWatercraftLocation]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteWatercraftLocation]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteWatercraftLocation]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteWatercraftLocation]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteWatercraftLocation]'))
