USE [DataMagnifier]
GO
/****** Object:  Table [Party].[WritingCompany]    Script Date: 10.11.2022 14:36:02 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Party].[WritingCompany]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Party].[FK_PartySK_WritingCompany]') AND parent_object_id = OBJECT_ID(N'[Party].[WritingCompany]'))
ALTER TABLE [Party].[WritingCompany]  WITH CHECK ADD  CONSTRAINT [FK_PartySK_WritingCompany] FOREIGN KEY([PartySK])
REFERENCES [Party].[PartyMaster] ([PartySK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Party].[FK_PartySK_WritingCompany]') AND parent_object_id = OBJECT_ID(N'[Party].[WritingCompany]'))
ALTER TABLE [Party].[WritingCompany] CHECK CONSTRAINT [FK_PartySK_WritingCompany]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Party].[FK_SourceSystemSK_WritingCompany]') AND parent_object_id = OBJECT_ID(N'[Party].[WritingCompany]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Party].[FK_SourceSystemSK_WritingCompany]') AND parent_object_id = OBJECT_ID(N'[Party].[WritingCompany]'))
