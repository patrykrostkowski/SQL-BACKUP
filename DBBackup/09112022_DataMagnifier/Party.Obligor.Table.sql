USE [DataMagnifier]
GO
/****** Object:  Table [Party].[Obligor]    Script Date: 10.11.2022 14:36:02 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Party].[Obligor]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Party].[FK_PartySK_Obligor]') AND parent_object_id = OBJECT_ID(N'[Party].[Obligor]'))
ALTER TABLE [Party].[Obligor]  WITH CHECK ADD  CONSTRAINT [FK_PartySK_Obligor] FOREIGN KEY([PartySK])
REFERENCES [Party].[PartyMaster] ([PartySK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Party].[FK_PartySK_Obligor]') AND parent_object_id = OBJECT_ID(N'[Party].[Obligor]'))
ALTER TABLE [Party].[Obligor] CHECK CONSTRAINT [FK_PartySK_Obligor]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Party].[FK_SourceSystemSK_Obligor]') AND parent_object_id = OBJECT_ID(N'[Party].[Obligor]'))
ALTER TABLE [Party].[Obligor]  WITH CHECK ADD  CONSTRAINT [FK_SourceSystemSK_Obligor] FOREIGN KEY([SourceSystemSK])
REFERENCES [TypeList].[SourceSystem] ([SourceSystemSK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Party].[FK_SourceSystemSK_Obligor]') AND parent_object_id = OBJECT_ID(N'[Party].[Obligor]'))
