USE [DataMagnifier]
GO
/****** Object:  Table [Reins].[SurplusShareTreaty]    Script Date: 10.11.2022 14:36:04 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Reins].[SurplusShareTreaty]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Reins].[FK_ProportionalTreatySK_SurplusShareTreaty]') AND parent_object_id = OBJECT_ID(N'[Reins].[SurplusShareTreaty]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Reins].[FK_ProportionalTreatySK_SurplusShareTreaty]') AND parent_object_id = OBJECT_ID(N'[Reins].[SurplusShareTreaty]'))
