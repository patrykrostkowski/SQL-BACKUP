USE [DataMagnifier]
GO
/****** Object:  Table [Claim].[MedicalCase]    Script Date: 10.11.2022 14:43:35 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Claim].[MedicalCase]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_ClaimantSK_MedicalCase]') AND parent_object_id = OBJECT_ID(N'[Claim].[MedicalCase]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_ClaimantSK_MedicalCase]') AND parent_object_id = OBJECT_ID(N'[Claim].[MedicalCase]'))
ALTER TABLE [Claim].[MedicalCase] CHECK CONSTRAINT [FK_ClaimantSK_MedicalCase]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_SourceSystemSK_MedicalCase]') AND parent_object_id = OBJECT_ID(N'[Claim].[MedicalCase]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Claim].[FK_SourceSystemSK_MedicalCase]') AND parent_object_id = OBJECT_ID(N'[Claim].[MedicalCase]'))
