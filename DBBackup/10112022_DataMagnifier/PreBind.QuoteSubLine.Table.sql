USE [DataMagnifier]
GO
/****** Object:  Table [PreBind].[QuoteSubLine]    Script Date: 10.11.2022 14:47:10 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PreBind].[QuoteSubLine]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteLineofBusinessSK_QuoteSubLine]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteSubLine]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteLineofBusinessSK_QuoteSubLine]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteSubLine]'))
