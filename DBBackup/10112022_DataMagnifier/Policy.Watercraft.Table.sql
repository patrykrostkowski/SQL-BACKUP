USE [DataMagnifier]
GO
/****** Object:  Table [Policy].[Watercraft]    Script Date: 10.11.2022 14:35:02 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Policy].[Watercraft]') AND type in (N'U'))
BEGIN
CREATE TABLE [Policy].[Watercraft](
	[WatercraftSK] [int] IDENTITY(2,1) NOT NULL,
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_PolicyInsurableObjectSK_Watercraft]') AND parent_object_id = OBJECT_ID(N'[Policy].[Watercraft]'))
ALTER TABLE [Policy].[Watercraft]  WITH CHECK ADD  CONSTRAINT [FK_PolicyInsurableObjectSK_Watercraft] FOREIGN KEY([PolicyInsurableObjectSK])
REFERENCES [Policy].[PolicyInsurableObject] ([PolicyInsurableObjectSK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_PolicyInsurableObjectSK_Watercraft]') AND parent_object_id = OBJECT_ID(N'[Policy].[Watercraft]'))
ALTER TABLE [Policy].[Watercraft] CHECK CONSTRAINT [FK_PolicyInsurableObjectSK_Watercraft]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_SourceSystemSK_Watercraft]') AND parent_object_id = OBJECT_ID(N'[Policy].[Watercraft]'))
ALTER TABLE [Policy].[Watercraft]  WITH CHECK ADD  CONSTRAINT [FK_SourceSystemSK_Watercraft] FOREIGN KEY([SourceSystemSK])
REFERENCES [TypeList].[SourceSystem] ([SourceSystemSK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_SourceSystemSK_Watercraft]') AND parent_object_id = OBJECT_ID(N'[Policy].[Watercraft]'))
ALTER TABLE [Policy].[Watercraft] CHECK CONSTRAINT [FK_SourceSystemSK_Watercraft]
GO
