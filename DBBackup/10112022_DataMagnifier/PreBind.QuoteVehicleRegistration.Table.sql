USE [DataMagnifier]
GO
/****** Object:  Table [PreBind].[QuoteVehicleRegistration]    Script Date: 10.11.2022 14:20:45 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleRegistration]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteVehicleSK_QuoteVehicleRegistration]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleRegistration]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteVehicleSK_QuoteVehicleRegistration]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleRegistration]'))
ALTER TABLE [PreBind].[QuoteVehicleRegistration] CHECK CONSTRAINT [FK_QuoteVehicleSK_QuoteVehicleRegistration]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteVehicleRegistration]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleRegistration]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteVehicleRegistration]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleRegistration]'))
