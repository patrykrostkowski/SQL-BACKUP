USE [DataMagnifier]
GO
/****** Object:  Table [PreBind].[QuoteProperty]    Script Date: 10.11.2022 14:47:09 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PreBind].[QuoteProperty]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteInsurableObjectSK_QuoteProperty]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteProperty]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteInsurableObjectSK_QuoteProperty]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteProperty]'))
ALTER TABLE [PreBind].[QuoteProperty] CHECK CONSTRAINT [FK_QuoteInsurableObjectSK_QuoteProperty]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteProperty]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteProperty]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteProperty]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteProperty]'))
ALTER TABLE [PreBind].[QuoteProperty] CHECK CONSTRAINT [FK_SourceSystemSK_QuoteProperty]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_StateSK_QuoteProperty]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteProperty]'))
ALTER TABLE [PreBind].[QuoteProperty]  WITH CHECK ADD  CONSTRAINT [FK_StateSK_QuoteProperty] FOREIGN KEY([StateSK])
REFERENCES [TypeList].[State] ([StateSK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_StateSK_QuoteProperty]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteProperty]'))
