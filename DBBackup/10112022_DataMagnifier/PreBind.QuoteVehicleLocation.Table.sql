USE [DataMagnifier]
GO
/****** Object:  Table [PreBind].[QuoteVehicleLocation]    Script Date: 10.11.2022 14:43:37 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleLocation]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteVehicleSK_QuoteVehicleLocation]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleLocation]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuoteVehicleSK_QuoteVehicleLocation]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleLocation]'))
ALTER TABLE [PreBind].[QuoteVehicleLocation] CHECK CONSTRAINT [FK_QuoteVehicleSK_QuoteVehicleLocation]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteVehicleLocation]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleLocation]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteVehicleLocation]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteVehicleLocation]'))
