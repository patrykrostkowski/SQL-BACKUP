USE [DataMagnifier]
GO
/****** Object:  Table [TypeList].[NorthAmericanIndustryClassification]    Script Date: 10.11.2022 14:20:46 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[TypeList].[NorthAmericanIndustryClassification]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[TypeList].[FK_SourceSystemSK_NorthAmericanIndustryClassification]') AND parent_object_id = OBJECT_ID(N'[TypeList].[NorthAmericanIndustryClassification]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[TypeList].[FK_SourceSystemSK_NorthAmericanIndustryClassification]') AND parent_object_id = OBJECT_ID(N'[TypeList].[NorthAmericanIndustryClassification]'))
