USE [DataMagnifier]
GO
/****** Object:  Table [Policy].[AccidentViolation]    Script Date: 10.11.2022 14:35:02 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Policy].[AccidentViolation]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_DriverSK_AccidentViolation]') AND parent_object_id = OBJECT_ID(N'[Policy].[AccidentViolation]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_DriverSK_AccidentViolation]') AND parent_object_id = OBJECT_ID(N'[Policy].[AccidentViolation]'))
ALTER TABLE [Policy].[AccidentViolation] CHECK CONSTRAINT [FK_DriverSK_AccidentViolation]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_SourceSystemSK_AccidentViolation]') AND parent_object_id = OBJECT_ID(N'[Policy].[AccidentViolation]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_SourceSystemSK_AccidentViolation]') AND parent_object_id = OBJECT_ID(N'[Policy].[AccidentViolation]'))
