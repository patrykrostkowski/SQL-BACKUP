USE [DataMagnifier]
GO
/****** Object:  Table [Survey].[QuoteQuestionAnswer]    Script Date: 10.11.2022 14:35:03 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Survey].[QuoteQuestionAnswer]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Survey].[FK_DomainTypeSK_QuoteQuestionAnswer]') AND parent_object_id = OBJECT_ID(N'[Survey].[QuoteQuestionAnswer]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Survey].[FK_DomainTypeSK_QuoteQuestionAnswer]') AND parent_object_id = OBJECT_ID(N'[Survey].[QuoteQuestionAnswer]'))
