USE [DataMagnifier]
GO
/****** Object:  Table [PreBind].[QuoteDwelling]    Script Date: 10.11.2022 14:47:09 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PreBind].[QuoteDwelling]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuotePropertySK_QuoteDwelling]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteDwelling]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_QuotePropertySK_QuoteDwelling]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteDwelling]'))
ALTER TABLE [PreBind].[QuoteDwelling] CHECK CONSTRAINT [FK_QuotePropertySK_QuoteDwelling]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteDwelling]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteDwelling]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[PreBind].[FK_SourceSystemSK_QuoteDwelling]') AND parent_object_id = OBJECT_ID(N'[PreBind].[QuoteDwelling]'))
