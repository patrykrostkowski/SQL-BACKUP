USE [DataMagnifier]
GO
/****** Object:  Table [Finance].[ExchangeRate]    Script Date: 10.11.2022 14:35:02 ******/
SET ANSI_NULLS ON
GO
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Finance].[FK_SourceSystemSK_ExchangeRate]') AND parent_object_id = OBJECT_ID(N'[Finance].[ExchangeRate]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Finance].[FK_SourceSystemSK_ExchangeRate]') AND parent_object_id = OBJECT_ID(N'[Finance].[ExchangeRate]'))
