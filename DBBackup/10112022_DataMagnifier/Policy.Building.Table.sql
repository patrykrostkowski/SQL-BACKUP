USE [DataMagnifier]
GO
/****** Object:  Table [Policy].[Building]    Script Date: 10.11.2022 14:43:36 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Policy].[Building]') AND type in (N'U'))
BEGIN
...
END
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_PropertySK_Building]') AND parent_object_id = OBJECT_ID(N'[Policy].[Building]'))
...
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_PropertySK_Building]') AND parent_object_id = OBJECT_ID(N'[Policy].[Building]'))
ALTER TABLE [Policy].[Building] CHECK CONSTRAINT [FK_PropertySK_Building]
GO
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_SourceSystemSK_Building]') AND parent_object_id = OBJECT_ID(N'[Policy].[Building]'))
ALTER TABLE [Policy].[Building]  WITH CHECK ADD  CONSTRAINT [FK_SourceSystemSK_Building] FOREIGN KEY([SourceSystemSK])
REFERENCES [TypeList].[SourceSystem] ([SourceSystemSK])
GO
IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[Policy].[FK_SourceSystemSK_Building]') AND parent_object_id = OBJECT_ID(N'[Policy].[Building]'))
